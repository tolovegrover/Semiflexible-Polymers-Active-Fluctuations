...
def main():
    ...
    def collect_series(tau, acts):
        ...
        for act in sorted(acts):
            ...
            if (tau, act) in fit_pars and (tau, act) in cov_data:
                ...
                fit_nn_x.append(act)
                fit_nn_y.append(np.sqrt(max(rg2_f_nn, 0)))
                
        return (true_x, true_y), (disc_x, disc_y), (fit_x, fit_y), (fit_nn_x, fit_nn_y)
    ...
    for i, tau in enumerate(unique_taus):
        ...
        (tx, ty), (dx, dy), (fx, fy), (fnn_x, fnn_y) = collect_series(tau, acts_t)
