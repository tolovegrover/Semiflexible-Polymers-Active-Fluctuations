...
DUMP_FOLDERS = sys.argv[1:] if len(sys.argv) > 1 else ["/home/love/apfs/root/kappa32/position"]
    ...
    if cov_dict is None:
        ...
        for tau in unique_taus:
            ...
            for act in acts:
                for folder in DUMP_FOLDERS:
                    pat = f"{folder}/position.64_32_{act:.0f}_0.1_0.1_{tau_str}_*.dump"
        ...
        if tasks:
            ...
            for key, data in agg_cov.items():
                count = data['count']
                tau, act = key
                print(f"Aggregated {count} seeds for tau={tau}, act={act}")
                cov_dict[key] = (data['C'] / count, data['rg2'] / count)
